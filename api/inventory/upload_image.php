...
try {
    ...
    if (!$img64) { Response::validationError(['image_base64'=>'Requerida']); }
    $product = $db->selectOne('SELECT product_id FROM products WHERE product_id = ?',[$product_id]);
    if(!$product){ Response::notFound('Producto no existe'); }
    ...
    if ($data_bin === false) { Response::validationError(['image_base64'=>'Base64 corrupto']); }
    if (strlen($data_bin) > 2*1024*1024) { Response::validationError(['image_base64'=>'Máximo 2MB']); }

    $filename = 'p_'.$product_id.'_'.time().'.'.$ext;
    $targetDir = realpath(__DIR__.'/../../public/assets/images/products');
    if (!$targetDir) { Response::error('Directorio imágenes no existe',500); }
