...
try {
    ...
    try {
        ...
        $sale_id = $db->insert('INSERT INTO sales (store_id, user_id, customer_id, register_id, sale_date, subtotal, tax, discount, total, amount_paid, payment_method, codi_payment_id, stripe_payment_id, status, created_via, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())',[
            $store_id, $user['user_id'], ($customer_id > 0 ? $customer_id : null), $register_id, date('Y-m-d H:i:s'), $subtotal, $tax, $discount, $total, $amount_paid, $payment_method, $codi_payment_id, $stripePaymentId, SALE_COMPLETED, $createdVia
        ]);
