...
function productCard(p, selected) {
    ...
    return `<button type="button" class="purchase-product-card ${selected?'selected':''}" data-product-id="${p.product_id}" aria-pressed="${selected}"><span class="product-check"><i class="fas ${selected?'fa-check':'fa-plus'}"></i></span><img src="${esc(p.image_path||'assets/images/products/default-product.svg')}" onerror="this.src='assets/images/products/default-product.svg'" alt=""><span class="product-card-name">${esc(p.product_name)}</span><span class="product-card-type">${type} · Stock ${p.available??p.current_stock??0}</span></button>`;
}
...
function renderSelectedPurchaseItems() {
    ...
    target.innerHTML=selectedPurchaseProducts.size?[...selectedPurchaseProducts.values()].map(p=>`<div class="selected-purchase-row" data-selected-id="${p.product_id}"><div class="selected-purchase-name"><i class="fas ${p.tracking_type==='component'?'fa-cubes':'fa-box'}"></i><strong>${esc(p.product_name)}</strong><small>${p.tracking_type==='component'?'Componente':'Producto final'}</small></div><label>Cantidad<input type="number" min="0.001" step="0.001" data-plan-qty="${p.product_id}" value="${p.planned_quantity||1}"></label><label>Costo estimado<input type="number" min="0" step="0.01" data-plan-cost="${p.product_id}" value="${p.unit_cost||0}"></label><button type="button" class="icon-button danger" data-remove-selected="${p.product_id}" aria-label="Quitar producto"><i class="fas fa-trash"></i></button></div>`).join(''):'<div class="selected-empty">Selecciona uno o varios productos del catálogo</div>';
    ...
    target.querySelectorAll('[data-plan-cost]').forEach(i=>i.addEventListener('input',()=>{const p=selectedPurchaseProducts.get(Number(i.dataset.planCost));if(p)p.unit_cost=i.value;}));
}
function togglePurchaseProduct(id) {
    const p=purchaseProducts.find(x=>x.product_id===id); if(!p)return;
    if(selectedPurchaseProducts.has(id))selectedPurchaseProducts.delete(id); else selectedPurchaseProducts.set(id,{...p,planned_quantity:1,unit_cost:Number(p.cost)||0});
...
async function savePurchaseComposer(existingId) {
    const modal=document.getElementById('purchaseComposerModal');
    const items=[...selectedPurchaseProducts.values()].map(p=>({product_id:Number(p.product_id),planned_quantity:Number(p.planned_quantity),unit_cost:Number(p.unit_cost)||0}));
    ...
    try {
        ...
        else { for(const item of items) { if(item.item_id) await api('../api/purchases/purchases.php',{method:'PUT',body:JSON.stringify({purchase_id:id,action:'update_item',item_id:Number(item.item_id),planned_quantity:item.planned_quantity,unit_cost:item.unit_cost})}); else await api('../api/purchases/purchases.php',{method:'PUT',body:JSON.stringify({purchase_id:id,action:'add_item',product_id:item.product_id,planned_quantity:item.planned_quantity})}); } }
...
function renderPurchaseDetail(p) {
    ...
    const body=`<div class="purchase-detail-meta"><div><strong>Proveedor:</strong> ${esc(p.supplier_name||'Sin proveedor')}</div><div><strong>Creada:</strong> ${dateText(p.created_at)}</div><div><strong>Estado:</strong> <span class="purchase-status" style="background:${PURCHASE_STATUS_COLORS[p.status]}">${esc(PURCHASE_STATUS_LABELS[p.status])}</span></div><div><strong>Total:</strong> ${money(p.total_cost)}</div></div><div class="purchase-detail-items">${p.items.length?p.items.map(i=>`<div class="detail-item-row"><div class="detail-item-name"><strong>${esc(i.product_name)}</strong><small>${i.tracking_type==='component'?'Componente':'Producto final'}</small></div>${editable&&!i.actual_quantity?`<label>Cantidad<input type="number" min="0.001" step="0.001" data-edit-qty="${i.item_id}" value="${i.planned_quantity}"></label><label>Costo<input type="number" min="0" step="0.01" data-edit-cost="${i.item_id}" value="${i.unit_cost}"></label><button type="button" class="icon-button danger" data-remove-detail="${i.item_id}" aria-label="Quitar"><i class="fas fa-trash"></i></button>`:`<span>Recibido: ${i.actual_quantity??'—'}</span><strong>${money(i.total_cost)}</strong>`}</div>`).join(''):'<div class="empty-state">Esta orden no tiene productos</div>'}</div>`;
    let actions='';
    if(editable)actions+=`<button type="button" class="btn-secondary" id="detailAddProducts"><i class="fas fa-plus"></i> Agregar productos</button>`;
    if(p.status==='pending')actions+=`<button type="button" class="btn-primary" id="detailExecute"><i class="fas fa-box-open"></i> Recibir compra</button>`;
    if(editable)actions+=`<button type="button" class="btn-danger-outline" id="detailCancel"><i class="fas fa-ban"></i> Cancelar orden</button>`;
    ...
    modal.querySelector('#detailExecute')?.addEventListener('click',()=>renderExecutionInsideModal(p));
...
function renderExecutionInsideModal(p) {
    const modal=document.getElementById('purchaseDetailModal'); if(!modal)return;
    const body=modal.querySelector('.modal-body');
    body.innerHTML=`<p class="execution-intro"><i class="fas fa-circle-info"></i> Captura lo recibido. Los productos que no llegaron pueden dejarse en cero.</p><div class="execute-items">${p.items.map(i=>`<div class="execute-item-row" data-item-id="${i.item_id}"><strong>${esc(i.product_name)}</strong><small>${i.tracking_type==='component'?'Componente':'Producto final'} · Planeado ${i.planned_quantity}</small><div class="execute-item-fields"><label>Cantidad recibida<input type="number" min="0" step="0.001" class="execute-qty" value="${i.planned_quantity}"></label><label>Costo pagado por unidad<input type="number" min="0" step="0.01" class="execute-cost" value="${i.unit_cost||0}"></label><strong class="execute-total">$0.00</strong></div></div>`).join('')}</div><div class="execute-summary">Total: <strong id="executeGrandTotal">$0.00</strong></div>`;
    modal.querySelector('.modal-footer').innerHTML='<button type="button" class="btn-secondary" id="backToPurchaseDetail">Volver</button><button type="button" class="btn-primary" id="confirmExecuteButton"><i class="fas fa-check"></i> Registrar recepción</button>';
    modal.querySelector('#backToPurchaseDetail').addEventListener('click',()=>{modal.remove();openPurchaseDetail(p.purchase_id);});
    const recalc=()=>{let total=0;modal.querySelectorAll('.execute-item-row').forEach(row=>{const q=Number(row.querySelector('.execute-qty').value)||0,c=Number(row.querySelector('.execute-cost').value)||0;total+=q*c;row.querySelector('.execute-total').textContent=money(q*c);});modal.querySelector('#executeGrandTotal').textContent=money(total);};
    modal.querySelectorAll('input').forEach(i=>i.addEventListener('input',recalc)); recalc();
    modal.querySelector('#confirmExecuteButton').addEventListener('click',()=>decisionModal('Registrar recepción','Se actualizarán existencias y caja con los datos capturados.','Registrar recepción',()=>confirmExecution(p.purchase_id),false));
}
async function confirmExecution(id) {
    const modal=document.getElementById('purchaseDetailModal');const items=[...modal.querySelectorAll('.execute-item-row')].map(row=>({item_id:Number(row.dataset.itemId),actual_quantity:Number(row.querySelector('.execute-qty').value)||0,unit_cost:Number(row.querySelector('.execute-cost').value)||0})).filter(i=>i.actual_quantity>0);
    if(!items.length){notify('Captura al menos un producto recibido','error');return;}
    try{const data=await api('../api/purchases/purchases.php',{method:'PUT',body:JSON.stringify({purchase_id:id,action:'execute',items})});modal.remove();loadPurchases();loadMovements();notify(`Compra recibida por ${money(data.data.total_cost)}`,'success');}catch(e){notify(e.message,'error');}
}
